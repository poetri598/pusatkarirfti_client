...
import { getOrganizationExperiencesByUsername, deleteOrganizationExperiencesByUsername } from "@/services/userOrganizationExperience";
...
export default function Page() {
  ...
  useEffect(() => {
    const fetchExperiences = async () => {
      ...
      const { success, data, error } = await getOrganizationExperiencesByUsername(user.user_name);

      if (success && data) {
        const mapped = data.map((exp: UserOrganizationExperienceItem) => ({
      ...
      <div className="w-full justify-end flex xs:flex-col md:flex-row gap-2">
        <Button
          ...
          onPress={async () => {
            ...
            const result = await deleteOrganizationExperiencesByUsername(user?.user_name || "");
            if (result.success) {
        ...
        <Link className="login" href={`/buat-cv/ubah-pengalaman-organisasi/${user?.user_name}`}>
