...
export default function Page({ user_name }: { user_name: string }) {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      const { success, data, error } = await getOrganizationExperiencesByUsername(user_name);
      if (success && data) {
        const mapped: OrganizationExperienceInput[] = data.map((exp: UserOrganizationExperienceItem) => ({
          ...
          user_organization_experience_descriptions:
            typeof exp.user_organization_experience_descriptions === "string"
              ? (exp.user_organization_experience_descriptions as string).split("@")
              : exp.user_organization_experience_descriptions?.map((desc: any) => desc.user_organization_experience_description_name) ?? [""],
        }));
  ...
  const handleChange = <K extends keyof OrganizationExperienceInput>(expIndex: number, key: K, value: OrganizationExperienceInput[K]) => {
  ...
  const handleDescChange = (expIndex: number, descIndex: number, value: string) => {
    const updated = [...experiences];
    updated[expIndex].user_organization_experience_descriptions[descIndex] = value;
    setExperiences(updated);
  };
  ...
  const addDescription = (expIndex: number) => {
    const updated = [...experiences];
    updated[expIndex].user_organization_experience_descriptions.push("");
    setExperiences(updated);
  };

  const removeDescription = (expIndex: number, descIndex: number) => {
    const updated = [...experiences];
    updated[expIndex].user_organization_experience_descriptions.splice(descIndex, 1);
    setExperiences(updated);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { success, data, error } = await updateOrganizationExperiencesByUsername(user_name, formData);
    if (success) {
    ...
    <section className="w-full bg-background-primary py-8">
      <div className="xs:w-11/12 lg:w-10/12 min-h-screen mx-auto flex flex-col xs:gap-4 md:gap-8">
        ...
        >
          ...
          <BreadcrumbItem href={`/buat-cv/ubah-pengalaman-organisasi/${user_name}`} className="text-primary-primary">
              ...
              <Form className="w-full flex flex-col gap-4" onSubmit={handleSubmit}>
                  ...
                  <div key={i} className="relative w-full border border-default-200 rounded-medium p-4 flex flex-col gap-4">
                    <div className="w-full xs:justify-start md:justify-end flex ">
                      <Button type="button" className="delete" onPress={() => removeExperience(i)}>
                        Hapus Pengalaman Organisasi <Trash size={20} color="currentColor" className="text-white" />
                      </Button>
                    </div>
                    ...
                    <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="flex flex-col gap-4">
                        ...
                        <div className="flex flex-col gap-2">
                          {exp.user_organization_experience_descriptions.map((desc, j) => (
                        ...
                        {/* Current */}
                        <Switch
                          isSelected={exp.user_organization_experience_is_current}
                          onValueChange={(val) => handleChange(i, "user_organization_experience_is_current", val)}
                          classNames={{
                            thumb: "bg-primary-primary",
                            label: "text-xs font-medium text-text-primary",
                          }}
                        >
                          {exp.user_organization_experience_is_current ? "Masih menjabat" : "Sudah tidak menjabat"}
                        </Switch>
                      </div>
                ...
                <div className="w-full justify-start flex ">
                  <Button className="login" type="button" onPress={addExperience}>
                    Tambah Pengalaman Organisasi <AddCircle size={20} color="currentColor" className="text-white" />
                  </Button>
                </div>
