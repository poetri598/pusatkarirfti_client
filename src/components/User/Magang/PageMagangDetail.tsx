...
import { getInternshipBySlug, getAllInternshipsExceptSlug, incrementInternshipView } from "@/services/internship";
import RichTextDisplay from "@/components/Custom/RichTextDisplay";

export default function PageMagangDetail({ internship_slug }: { internship_slug: string }) {
  const [internship, setInternship] = useState<InternshipItem | null>(null);
  const [relatedInternships, setRelatedInternships] = useState<InternshipItem[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const relatedRes = await getAllInternshipsExceptSlug(internship_slug);
        if (relatedRes.success && relatedRes.data) {
          setRelatedInternships(relatedRes.data);
        } else {
  ...
  return (
    <main className="xs:w-11/12 lg:w-10/12 flex flex-col mx-auto bg-background-primary py-8 gap-8">
      ...
      <section className="flex flex-col gap-8 bg-background-primary ">
        <TitleSection label="Lainnya" href="/internship" />
        {/* Card */}
