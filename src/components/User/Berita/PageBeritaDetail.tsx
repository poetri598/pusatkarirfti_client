...
import { getNewsBySlug, getAllNewsExceptSlug, incrementNewsView } from "@/services/news";
...
export default function SectionBeritaClient({ news_slug }: { news_slug: string }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const relatedRes = await getAllNewsExceptSlug(news_slug);
  ...
  const relativeDate = getRelativeTimeRaw(news.news_created_at);
  const fullDate = getFullTimeRaw(news.news_created_at);
  return (
