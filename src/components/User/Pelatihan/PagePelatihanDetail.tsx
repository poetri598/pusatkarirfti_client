...
import { getTrainingBySlug, getAllTrainingsExceptSlug, incrementTrainingView } from "@/services/training";
...
export default function PagePelatihanDetail({ training_slug }: { training_slug: string }) {
  ...
  const [relatedTrainings, setRelatedTrainings] = useState<TrainingItem[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const relatedRes = await getAllTrainingsExceptSlug(training_slug);
        if (relatedRes.success && relatedRes.data) {
          setRelatedTrainings(relatedRes.data);
        } else {
          setRelatedTrainings([]);
        }
      } catch (error) {
    ...
    <main className="xs:w-11/12 lg:w-10/12 flex flex-col mx-auto bg-background-primary py-8 gap-8">
      <section className="flex flex-col xs:gap-2">
        ...
        <section className="flex flex-col xs:gap-4 md:gap-8">
          ...
          <div className="flex items-center gap-2">
            <Image src={training?.company_img} alt={training?.company_name} width={72} height={72} className="object-contain" />
            <span className="xs:text-sm md:text-base font-bold">{training?.company_name}</span>
          </div>
      ...
      <section className="flex flex-col gap-8 bg-background-primary ">
        ...
        <section className="grid xs:grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xs:gap-2 lg:gap-8">
          {relatedTrainings.length === 0 ? <p className="text-start text-xs text-text-secondary">Data belum tersedia</p> : relatedTrainings.map((item) => <CardPelatihan key={item.training_id} {...item} />)}
        </section>
      </section>
    </main>
  );
