...
// Services
import { deleteMaritalStatusById } from "@/services/maritalStatus";

// Utils
import { getRelativeTimeRaw, getFullTimeRaw } from "@/utils/time";

export default function page(props: MaritalStatusItem) {
  const { marital_status_id, marital_status_name, marital_status_created_at } = props;
  const relativeDate = getRelativeTimeRaw(marital_status_created_at);
  const fullDate = getFullTimeRaw(marital_status_created_at);
        ...
        <div className="flex items-center justify-end gap-2 w-full">
          ...
          >
            ...
            <DropdownMenu aria-label="Actions" classNames={{ base: "w-fit" }}>
              <DropdownItem href={`/status-perkawinan/${marital_status_id}`} key="lihat" classNames={{ base: "!w-fit" }}>
              ...
              <DropdownItem href={`/status-perkawinan/edit/${marital_status_id}`} key="edit" classNames={{ base: "!w-fit" }}>
                  ...
                  <Button
                    ...
                    onPress={async () => {
                      ...
                      if (confirm.isConfirmed) {
                        const result = await deleteMaritalStatusById(marital_status_id);
                        if (result.success) {
