...
// Services
import { deleteStatusById } from "@/services/status";

// Utils
import { getRelativeTimeRaw, getFullTimeRaw } from "@/utils/time";

export default function page(props: StatusItem) {
  const { status_id, status_name, status_created_at } = props;
  const relativeDate = getRelativeTimeRaw(status_created_at);
  const fullDate = getFullTimeRaw(status_created_at);
  ...
  return (
    <>
      <div className="w-full flex flex-col gap-4 p-4 mb-8 border border-default-200 rounded-medium hover:bg-default-200 overflow-hidden">
        <div className="flex items-center justify-end gap-2 w-full">
          ...
          >
            ...
            <DropdownMenu aria-label="Actions" classNames={{ base: "w-fit" }}>
              <DropdownItem href={`/status/${status_id}`} key="lihat" classNames={{ base: "!w-fit" }}>
              ...
              <DropdownItem href={`/status/edit/${status_id}`} key="edit" classNames={{ base: "!w-fit" }}>
                  ...
                  <Button
                    ...
                    onPress={async () => {
                      ...
                      if (confirm.isConfirmed) {
                        const result = await deleteStatusById(status_id);
                        if (result.success) {
        ...
        <span className="text-xl font-bold">{status_name}</span>
