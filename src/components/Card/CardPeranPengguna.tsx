...
// Services
import { deleteRoleById } from "@/services/role";

// Utils
import { getRelativeTimeRaw, getFullTimeRaw } from "@/utils/time";

export default function page(props: RoleItem) {
  const { role_id, role_name, role_created_at } = props;
  const relativeDate = getRelativeTimeRaw(role_created_at);
  const fullDate = getFullTimeRaw(role_created_at);
  ...
  return (
    <>
      <div className="w-full flex flex-col gap-4 p-4 mb-8 border border-default-200 rounded-medium hover:bg-default-200 overflow-hidden">
        <div className="flex items-center justify-end gap-2 w-full">
          ...
          >
            ...
            <DropdownMenu aria-label="Actions" classNames={{ base: "w-fit" }}>
              <DropdownItem href={`/peran-pengguna/${role_id}`} key="lihat" classNames={{ base: "!w-fit" }}>
              ...
              <DropdownItem href={`/peran-pengguna/edit/${role_id}`} key="edit" classNames={{ base: "!w-fit" }}>
                  ...
                  <Button
                    ...
                    onPress={async () => {
                      ...
                      if (confirm.isConfirmed) {
                        const result = await deleteRoleById(role_id);
                        if (result.success) {
        ...
        <span className="text-xl font-bold">{role_name}</span>
