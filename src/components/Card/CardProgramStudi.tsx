...
// Services
import { deleteProgramStudyById } from "@/services/programStudy";

// Utils
import { getRelativeTimeRaw, getFullTimeRaw } from "@/utils/time";

export default function page(props: ProgramStudyItem) {
  const { program_study_id, program_study_name, program_study_created_at } = props;
  const relativeDate = getRelativeTimeRaw(program_study_created_at);
  const fullDate = getFullTimeRaw(program_study_created_at);
  ...
  return (
    <>
      <div className="w-full flex flex-col gap-4 p-4 mb-8 border border-default-200 rounded-medium hover:bg-default-200 overflow-hidden">
        <div className="flex items-center justify-end gap-2 w-full">
          ...
          >
            ...
            <DropdownMenu aria-label="Actions" classNames={{ base: "w-fit" }}>
              <DropdownItem href={`/program-studi/${program_study_id}`} key="lihat" classNames={{ base: "!w-fit" }}>
              ...
              <DropdownItem href={`/program-studi/edit/${program_study_id}`} key="edit" classNames={{ base: "!w-fit" }}>
                  ...
                  <Button
                    ...
                    onPress={async () => {
                      ...
                      if (confirm.isConfirmed) {
                        const result = await deleteProgramStudyById(program_study_id);
                        if (result.success) {
        ...
        <span className="text-xl font-bold">{program_study_name}</span>
