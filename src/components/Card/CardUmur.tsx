...
import { deleteAgeById } from "@/services/age";
import { showConfirmationDialog, showSuccessDialog, showErrorDialog } from "@/components/Custom/AlertButton";

// Types
import { AgeItem } from "@/types/age";

export default function CardPengguna(props: AgeItem) {
  const { age_id, age_no, age_created_at } = props;
  const relativeDate = getRelativeTimeRaw(age_created_at);
  const fullDate = getFullTimeRaw(age_created_at);
  ...
  return (
    <>
      <div className="w-full flex flex-col gap-4 p-4 mb-8 border border-default-200 rounded-medium hover:bg-default-200 overflow-hidden">
        <div className="flex items-center justify-end gap-2 w-full">
          ...
          >
            ...
            <DropdownMenu aria-label="Actions" classNames={{ base: "w-fit" }}>
              <DropdownItem href={`/umur/${age_id}`} key="lihat" classNames={{ base: "!w-fit" }}>
              ...
              <DropdownItem href={`/umur/edit/${age_id}`} key="edit" classNames={{ base: "!w-fit" }}>
                  ...
                  <Button
                    ...
                    onPress={async () => {
                      ...
                      if (confirm.isConfirmed) {
                        const result = await deleteAgeById(age_id);
                        if (result.success) {
        ...
        {/* Umur */}
        <span className="text-xl font-bold">{age_no}</span>
        {/* Tanggal */}
