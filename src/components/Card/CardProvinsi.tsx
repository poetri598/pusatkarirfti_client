...
// Services
import { deleteProvinceById } from "@/services/province";

// Utils
import { getRelativeTimeRaw, getFullTimeRaw } from "@/utils/time";

export default function page(props: ProvinceItem) {
  const { province_id, province_name, province_created_at } = props;
  const relativeDate = getRelativeTimeRaw(province_created_at);
  const fullDate = getFullTimeRaw(province_created_at);
  ...
  return (
    <>
      <div className="w-full flex flex-col gap-4 p-4 mb-8 border border-default-200 rounded-medium hover:bg-default-200 overflow-hidden">
        <div className="flex items-center justify-end gap-2 w-full">
          ...
          >
            ...
            <DropdownMenu aria-label="Actions" classNames={{ base: "w-fit" }}>
              <DropdownItem href={`/provinsi/${province_id}`} key="lihat" classNames={{ base: "!w-fit" }}>
              ...
              <DropdownItem href={`/provinsi/edit/${province_id}`} key="edit" classNames={{ base: "!w-fit" }}>
                  ...
                  <Button
                    ...
                    onPress={async () => {
                      ...
                      if (confirm.isConfirmed) {
                        const result = await deleteProvinceById(province_id);
                        if (result.success) {
        ...
        <span className="text-xl font-bold">{province_name}</span>
