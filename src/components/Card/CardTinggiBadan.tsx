...
// Services
import { deleteHeightById } from "@/services/height";

// Utils
import { getRelativeTimeRaw, getFullTimeRaw } from "@/utils/time";

export default function page(props: HeightItem) {
  const { height_id, height_no, height_created_at } = props;
  const relativeDate = getRelativeTimeRaw(height_created_at);
  const fullDate = getFullTimeRaw(height_created_at);
  ...
  return (
    <>
      <div className="w-full flex flex-col gap-4 p-4 mb-8 border border-default-200 rounded-medium hover:bg-default-200 overflow-hidden">
        <div className="flex items-center justify-end gap-2 w-full">
          ...
          >
            ...
            <DropdownMenu aria-label="Actions" classNames={{ base: "w-fit" }}>
              <DropdownItem href={`/tinggi-badan/${height_id}`} key="lihat" classNames={{ base: "!w-fit" }}>
              ...
              <DropdownItem href={`/tinggi-badan/edit/${height_id}`} key="edit" classNames={{ base: "!w-fit" }}>
                  ...
                  <Button
                    ...
                    onPress={async () => {
                      ...
                      if (confirm.isConfirmed) {
                        const result = await deleteHeightById(height_id);
                        if (result.success) {
        ...
        {/* Umur */}
        <span className="text-xl font-bold">{height_no}</span>
        {/* Tanggal */}
