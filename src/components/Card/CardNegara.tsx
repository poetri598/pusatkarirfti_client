...
// Services
import { deleteCountryById } from "@/services/country";

// Utils
import { getRelativeTimeRaw, getFullTimeRaw } from "@/utils/time";

export default function page(props: CountryItem) {
  const { country_id, country_name, country_created_at } = props;
  const relativeDate = getRelativeTimeRaw(country_created_at);
  const fullDate = getFullTimeRaw(country_created_at);
        ...
        <div className="flex items-center justify-end gap-2 w-full">
          ...
          >
            ...
            <DropdownMenu aria-label="Actions" classNames={{ base: "w-fit" }}>
              <DropdownItem href={`/negara/${country_id}`} key="lihat" classNames={{ base: "!w-fit" }}>
              ...
              <DropdownItem href={`/negara/edit/${country_id}`} key="edit" classNames={{ base: "!w-fit" }}>
                  ...
                  <Button
                    ...
                    onPress={async () => {
                      ...
                      if (confirm.isConfirmed) {
                        const result = await deleteCountryById(country_id);
                        if (result.success) {
