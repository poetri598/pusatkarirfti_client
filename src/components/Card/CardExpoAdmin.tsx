...
import { deleteExpoById } from "@/services/expo";
  ...
  return (
    <>
      <div className="w-full flex flex-col gap-4 p-4 mb-8 border border-default-200 rounded-medium hover:bg-default-200 overflow-hidden">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          >
            ...
            <DropdownMenu aria-label="Actions" classNames={{ base: "w-fit" }}>
              <DropdownItem href={`/expo/${expo_slug}`} key="lihat" classNames={{ base: "!w-fit" }}>
              ...
              <DropdownItem href={`/expo/edit/${expo_slug}`} key="edit" classNames={{ base: "!w-fit" }}>
                  ...
                  <Button
                    ...
                    onPress={async () => {
                      ...
                      if (confirm.isConfirmed) {
                        const result = await deleteExpoById(expo_id);
                        if (result.success) {
        ...
        {/* Image*/}
        <Image src={expo_img} alt={expo_img} width={1280} height={720} className="w-full !h-48 aspect-video rounded-md object-contain" />

        {/* Nama */}
        <span className="text-text-primary text-xs font-bold">{expo_name}</span>
        ...
        <div className="flex xs:flex-col md:flex-row xs:items-start md:items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1 text-xs text-text-secondary">
            <Tooltip
              content={expo_views.toLocaleString("id-ID")}
              placement="top"
              classNames={{
                content: "text-xs text-background-primary bg-primary-primary",
              }}
            >
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs text-text-secondary">{formatViews(expo_views)} kali dilihat</span>
              </div>
            </Tooltip>
          </div>
        </div>
