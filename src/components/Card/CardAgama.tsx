...
// Services
import { deleteReligionById } from "@/services/religion";

// Utils
import { getRelativeTimeRaw, getFullTimeRaw } from "@/utils/time";

export default function page(props: ReligionItem) {
  const { religion_id, religion_name, religion_created_at } = props;
  const relativeDate = getRelativeTimeRaw(religion_created_at);
  const fullDate = getFullTimeRaw(religion_created_at);
  ...
  return (
    <>
      <div className="w-full flex flex-col gap-4 p-4 mb-8 border border-default-200 rounded-medium hover:bg-default-200 overflow-hidden">
        <div className="flex items-center justify-end gap-2 w-full">
          ...
          >
            ...
            <DropdownMenu aria-label="Actions" classNames={{ base: "w-fit" }}>
              <DropdownItem href={`/agama/${religion_id}`} key="lihat" classNames={{ base: "!w-fit" }}>
              ...
              <DropdownItem href={`/agama/edit/${religion_id}`} key="edit" classNames={{ base: "!w-fit" }}>
                  ...
                  <Button
                    ...
                    onPress={async () => {
                      ...
                      if (confirm.isConfirmed) {
                        const result = await deleteReligionById(religion_id);
                        if (result.success) {
        ...
        <span className="text-xl font-bold">{religion_name}</span>
