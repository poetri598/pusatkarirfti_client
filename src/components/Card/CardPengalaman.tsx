...
// Services
import { deleteExperienceById } from "@/services/experience";

// Utils
import { getRelativeTimeRaw, getFullTimeRaw } from "@/utils/time";

export default function page(props: ExperienceItem) {
  const { experience_id, experience_name, experience_created_at } = props;
  const relativeDate = getRelativeTimeRaw(experience_created_at);
  const fullDate = getFullTimeRaw(experience_created_at);
  ...
  return (
    <>
      <div className="w-full flex flex-col gap-4 p-4 mb-8 border border-default-200 rounded-medium hover:bg-default-200 overflow-hidden">
        <div className="flex items-center justify-end gap-2 w-full">
          ...
          >
            ...
            <DropdownMenu aria-label="Actions" classNames={{ base: "w-fit" }}>
              <DropdownItem href={`/pengalaman/${experience_id}`} key="lihat" classNames={{ base: "!w-fit" }}>
              ...
              <DropdownItem href={`/pengalaman/edit/${experience_id}`} key="edit" classNames={{ base: "!w-fit" }}>
                  ...
                  <Button
                    ...
                    onPress={async () => {
                      ...
                      if (confirm.isConfirmed) {
                        const result = await deleteExperienceById(experience_id);
                        if (result.success) {
        ...
        <span className="text-xl font-bold">{experience_name}</span>
