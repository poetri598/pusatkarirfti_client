...
// Services
import { deleteIpkById } from "@/services/ipk";

// Utils
import { getRelativeTimeRaw, getFullTimeRaw } from "@/utils/time";

export default function page(props: IpkItem) {
  const { ipk_id, ipk_no, ipk_created_at } = props;
  const relativeDate = getRelativeTimeRaw(ipk_created_at);
  const fullDate = getFullTimeRaw(ipk_created_at);
  ...
  return (
    <>
      <div className="w-full flex flex-col gap-4 p-4 mb-8 border border-default-200 rounded-medium hover:bg-default-200 overflow-hidden">
        <div className="flex items-center justify-end gap-2 w-full">
          ...
          >
            ...
            <DropdownMenu aria-label="Actions" classNames={{ base: "w-fit" }}>
              <DropdownItem href={`/ipk/${ipk_id}`} key="lihat" classNames={{ base: "!w-fit" }}>
              ...
              <DropdownItem href={`/ipk/edit/${ipk_id}`} key="edit" classNames={{ base: "!w-fit" }}>
                  ...
                  <Button
                    ...
                    onPress={async () => {
                      ...
                      if (confirm.isConfirmed) {
                        const result = await deleteIpkById(ipk_id);
                        if (result.success) {
        ...
        {/* Umur */}
        <span className="text-xl font-bold">{ipk_no}</span>
        {/* Tanggal */}
