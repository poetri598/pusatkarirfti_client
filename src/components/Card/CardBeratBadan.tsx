...
// Services
import { deleteWeightById } from "@/services/weight";

// Utils
import { getRelativeTimeRaw, getFullTimeRaw } from "@/utils/time";

export default function page(props: WeightItem) {
  const { weight_id, weight_no, weight_created_at } = props;
  const relativeDate = getRelativeTimeRaw(weight_created_at);
  const fullDate = getFullTimeRaw(weight_created_at);
  ...
  return (
    <>
      <div className="w-full flex flex-col gap-4 p-4 mb-8 border border-default-200 rounded-medium hover:bg-default-200 overflow-hidden">
        <div className="flex items-center justify-end gap-2 w-full">
          ...
          >
            ...
            <DropdownMenu aria-label="Actions" classNames={{ base: "w-fit" }}>
              <DropdownItem href={`/berat-badan/${weight_id}`} key="lihat" classNames={{ base: "!w-fit" }}>
              ...
              <DropdownItem href={`/berat-badan/edit/${weight_id}`} key="edit" classNames={{ base: "!w-fit" }}>
                  ...
                  <Button
                    ...
                    onPress={async () => {
                      ...
                      if (confirm.isConfirmed) {
                        const result = await deleteWeightById(weight_id);
                        if (result.success) {
        ...
        {/* Umur */}
        <span className="text-xl font-bold">{weight_no}</span>
        {/* Tanggal */}
