...
// Services
import { deleteNewsTypeById } from "@/services/newsType";

// Utils
import { getRelativeTimeRaw, getFullTimeRaw } from "@/utils/time";

export default function page(props: NewsTypeItem) {
  const { news_type_id, news_type_name, news_type_created_at } = props;
  const relativeDate = getRelativeTimeRaw(news_type_created_at);
  const fullDate = getFullTimeRaw(news_type_created_at);
  ...
  return (
    <>
      <div className="w-full flex flex-col gap-4 p-4 mb-8 border border-default-200 rounded-medium hover:bg-default-200 overflow-hidden">
        <div className="flex items-center justify-end gap-2 w-full">
          ...
          >
            ...
            <DropdownMenu aria-label="Actions" classNames={{ base: "w-fit" }}>
              <DropdownItem href={`/tipe-berita/${news_type_id}`} key="lihat" classNames={{ base: "!w-fit" }}>
              ...
              <DropdownItem href={`/tipe-berita/edit/${news_type_id}`} key="edit" classNames={{ base: "!w-fit" }}>
                  ...
                  <Button
                    ...
                    onPress={async () => {
                      ...
                      if (confirm.isConfirmed) {
                        const result = await deleteNewsTypeById(news_type_id);
                        if (result.success) {
        ...
        <span className="text-xl font-bold">{news_type_name}</span>
