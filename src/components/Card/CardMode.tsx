...
// Services
import { deleteModeById } from "@/services/mode";

// Utils
import { getRelativeTimeRaw, getFullTimeRaw } from "@/utils/time";

export default function page(props: ModeItem) {
  const { mode_id, mode_name, mode_created_at } = props;
  const relativeDate = getRelativeTimeRaw(mode_created_at);
  const fullDate = getFullTimeRaw(mode_created_at);
  ...
  return (
    <>
      <div className="w-full flex flex-col gap-4 p-4 mb-8 border border-default-200 rounded-medium hover:bg-default-200 overflow-hidden">
        <div className="flex items-center justify-end gap-2 w-full">
          ...
          >
            ...
            <DropdownMenu aria-label="Actions" classNames={{ base: "w-fit" }}>
              <DropdownItem href={`/mode/${mode_id}`} key="lihat" classNames={{ base: "!w-fit" }}>
              ...
              <DropdownItem href={`/mode/edit/${mode_id}`} key="edit" classNames={{ base: "!w-fit" }}>
                  ...
                  <Button
                    ...
                    onPress={async () => {
                      ...
                      if (confirm.isConfirmed) {
                        const result = await deleteModeById(mode_id);
                        if (result.success) {
        ...
        <span className="text-xl font-bold">{mode_name}</span>
