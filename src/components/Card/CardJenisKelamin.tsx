...
// Services
import { deleteGenderById } from "@/services/gender";

// Utils
import { getRelativeTimeRaw, getFullTimeRaw } from "@/utils/time";

export default function page(props: GenderItem) {
  const { gender_id, gender_name, gender_created_at } = props;
  const relativeDate = getRelativeTimeRaw(gender_created_at);
  const fullDate = getFullTimeRaw(gender_created_at);
  ...
  return (
    <>
      <div className="w-full flex flex-col gap-4 p-4 mb-8 border border-default-200 rounded-medium hover:bg-default-200 overflow-hidden">
        <div className="flex items-center justify-end gap-2 w-full">
          ...
          >
            ...
            <DropdownMenu aria-label="Actions" classNames={{ base: "w-fit" }}>
              <DropdownItem href={`/jenis-kelamin/${gender_id}`} key="lihat" classNames={{ base: "!w-fit" }}>
              ...
              <DropdownItem href={`/jenis-kelamin/edit/${gender_id}`} key="edit" classNames={{ base: "!w-fit" }}>
                  ...
                  <Button
                    ...
                    onPress={async () => {
                      ...
                      if (confirm.isConfirmed) {
                        const result = await deleteGenderById(gender_id);
                        if (result.success) {
        ...
        <span className="text-xl font-bold">{gender_name}</span>
