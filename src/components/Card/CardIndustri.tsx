...
// Services
import { deleteIndustryById } from "@/services/industry";

// Utils
import { getRelativeTimeRaw, getFullTimeRaw } from "@/utils/time";

export default function page(props: IndustryItem) {
  const { industry_id, industry_name, industry_created_at } = props;
  const relativeDate = getRelativeTimeRaw(industry_created_at);
  const fullDate = getFullTimeRaw(industry_created_at);
  ...
  return (
    <>
      <div className="w-full flex flex-col gap-4 p-4 mb-8 border border-default-200 rounded-medium hover:bg-default-200 overflow-hidden">
        <div className="flex items-center justify-end gap-2 w-full">
          ...
          >
            ...
            <DropdownMenu aria-label="Actions" classNames={{ base: "w-fit" }}>
              <DropdownItem href={`/industri/${industry_id}`} key="lihat" classNames={{ base: "!w-fit" }}>
              ...
              <DropdownItem href={`/industri/edit/${industry_id}`} key="edit" classNames={{ base: "!w-fit" }}>
                  ...
                  <Button
                    ...
                    onPress={async () => {
                      ...
                      if (confirm.isConfirmed) {
                        const result = await deleteIndustryById(industry_id);
                        if (result.success) {
        ...
        <span className="text-xl font-bold">{industry_name}</span>
