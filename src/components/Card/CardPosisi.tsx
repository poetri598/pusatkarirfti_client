...
// Services
import { deletePositionById } from "@/services/position";

// Utils
import { getRelativeTimeRaw, getFullTimeRaw } from "@/utils/time";

export default function page(props: PositionItem) {
  const { position_id, position_name, position_created_at } = props;
  const relativeDate = getRelativeTimeRaw(position_created_at);
  const fullDate = getFullTimeRaw(position_created_at);
  ...
  return (
    <>
      <div className="w-full flex flex-col gap-4 p-4 mb-8 border border-default-200 rounded-medium hover:bg-default-200 overflow-hidden">
        <div className="flex items-center justify-end gap-2 w-full">
          ...
          >
            ...
            <DropdownMenu aria-label="Actions" classNames={{ base: "w-fit" }}>
              <DropdownItem href={`/posisi/${position_id}`} key="lihat" classNames={{ base: "!w-fit" }}>
              ...
              <DropdownItem href={`/posisi/edit/${position_id}`} key="edit" classNames={{ base: "!w-fit" }}>
                  ...
                  <Button
                    ...
                    onPress={async () => {
                      ...
                      if (confirm.isConfirmed) {
                        const result = await deletePositionById(position_id);
                        if (result.success) {
        ...
        <span className="text-xl font-bold">{position_name}</span>
