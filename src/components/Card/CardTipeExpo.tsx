...
// Services
import { deleteExpoTypeById } from "@/services/expoType";

// Utils
import { getRelativeTimeRaw, getFullTimeRaw } from "@/utils/time";

export default function page(props: ExpoTypeItem) {
  const { expo_type_id, expo_type_name, expo_type_created_at } = props;
  const relativeDate = getRelativeTimeRaw(expo_type_created_at);
  const fullDate = getFullTimeRaw(expo_type_created_at);
  ...
  return (
    <>
      <div className="w-full flex flex-col gap-4 p-4 mb-8 border border-default-200 rounded-medium hover:bg-default-200 overflow-hidden">
        <div className="flex items-center justify-end gap-2 w-full">
          ...
          >
            ...
            <DropdownMenu aria-label="Actions" classNames={{ base: "w-fit" }}>
              <DropdownItem href={`/tipe-expo/${expo_type_id}`} key="lihat" classNames={{ base: "!w-fit" }}>
              ...
              <DropdownItem href={`/tipe-expo/edit/${expo_type_id}`} key="edit" classNames={{ base: "!w-fit" }}>
                  ...
                  <Button
                    ...
                    onPress={async () => {
                      ...
                      if (confirm.isConfirmed) {
                        const result = await deleteExpoTypeById(expo_type_id);
                        if (result.success) {
        ...
        <span className="text-xl font-bold">{expo_type_name}</span>
