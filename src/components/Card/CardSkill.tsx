...
export default function page(props: SkillItem) {
  const { skill_id, skill_name, skill_created_at } = props;
  const relativeDate = getRelativeTimeRaw(skill_created_at);
  const fullDate = getFullTimeRaw(skill_created_at);
  ...
  return (
    <>
      <div className="w-full flex flex-col gap-4 p-4 mb-8 border border-default-200 rounded-medium hover:bg-default-200 overflow-hidden">
        <div className="flex items-center justify-end gap-2 w-full">
          ...
          >
            ...
            <DropdownMenu aria-label="Actions" classNames={{ base: "w-fit" }}>
              <DropdownItem href={`/skill/${skill_id}`} key="lihat" classNames={{ base: "!w-fit" }}>
              ...
              <DropdownItem href={`/skill/edit/${skill_id}`} key="edit" classNames={{ base: "!w-fit" }}>
                  ...
                  <Button
                    ...
                    onPress={async () => {
                      ...
                      if (confirm.isConfirmed) {
                        const result = await deleteSkillById(skill_id);
                        if (result.success) {
        ...
        <span className="text-xl font-bold">{skill_name}</span>
