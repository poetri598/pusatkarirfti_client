...
import { getUserPlatformsByUsername } from "@/services/userPlatform";
import { getUserByUsername, updateUserProfileWithSocials } from "@/services/user";
...
export default function Page({ user_name }: { user_name: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await updateUserProfileWithSocials(formData);
    if (res.success) {
      await showSuccessDialog();
    } else {
      await showErrorDialog(res.error || "Terjadi kesalahan");
    }
    setLoading(false);
  };
    ...
    <>
      <>
        <section className="w-full  bg-background-primary py-8">
          <div className="xs:w-11/12 lg:w-10/12 mx-auto min-h-screen flex flex-col xs:gap-4 md:gap-8">
            {loading && (
