...
import CardKota from "@/components/Card/CardKota";
...
// Services
import { searchCities, deleteCityById } from "@/services/city";
// Utils
...
export default function page() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const { success, data, error } = await searchCities(queryParams);
        if (success) {
          setData(data || []);
        } else {
  ...
  const sortOptions = [
    ...
    { key: "city_name:asc", label: "A-Z" },
    { key: "city_name:desc", label: "Z-A" },
    { key: "city_created_at:desc", label: "Terbaru" },
    { key: "city_created_at:asc", label: "Terlama" },
    { key: "city_updated_at:desc", label: "Diubah Terakhir" },
    { key: "city_updated_at:asc", label: "Diubah Terawal" },
  ];
  ...
  const tableItems = currentItems.map((item, index) => ({
    ...
    city_created_at: getRelativeTimeRaw(item.city_created_at),
    actions: item,
  }));
  ...
  return (
    <main className="w-full mx-auto flex flex-col gap-4 xs:p-0 md:p-8 bg-background-primary overflow-hidden">
      ...
      {/* Section Title */}
      <TitleSectionAdmin label="Kota" href="/kota/tambah" />
      ...
      <div className="flex flex-col gap-2">
        {searchKeyword && (
          <div className="flex flex-col">
            ...
            <span className="text-xs text-text-secondary">
              Kami menemukan <span className="font-bold text-primary-primary">{data.length}</span> informasi Kota
            </span>
          </div>
        )}
      </div>
      ...
      {viewMode === "card" ? (
        <section className="grid xs:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {isLoading ? (
            ...
            currentItems.map((item) => <CardKota key={item.city_id} {...item} />)
          )}
        </section>
      ) : (
        <Table
                            ...
                            <Button
                              ...
                              onPress={async () => {
                                ...
                                if (confirm.isConfirmed) {
                                  const result = await deleteCityById(data.city_id);
                                  if (result.success) {
