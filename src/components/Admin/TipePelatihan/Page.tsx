...
import CardTipePelatihan from "@/components/Card/CardTipePelatihan";
...
// Services
import { searchTrainingTypes, deleteTrainingTypeById } from "@/services/trainingType";
// Utils
import { getRelativeTimeRaw } from "@/utils/time";

export default function page() {
  const [data, setData] = useState<TrainingTypeItem[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const { success, data, error } = await searchTrainingTypes(queryParams);
        if (success) {
          setData(data || []);
        } else {
  ...
  const sortOptions = [
    ...
    { key: "training_type_name:asc", label: "A-Z" },
    { key: "training_type_name:desc", label: "Z-A" },
    { key: "training_type_created_at:desc", label: "Terbaru" },
    { key: "training_type_created_at:asc", label: "Terlama" },
    { key: "training_type_updated_at:desc", label: "Diubah Terakhir" },
    { key: "training_type_updated_at:asc", label: "Diubah Terawal" },
  ];
  ...
  const columns = [
    { key: "no", label: "#" },
    { key: "training_type_name", label: "Nama Tipe Pelatihan" },
    { key: "training_type_created_at", label: "Tanggal Dibuat" },
  ...
  const tableItems = currentItems.map((item, index) => ({
    ...
    training_type_created_at: getRelativeTimeRaw(item.training_type_created_at),
    actions: item,
  }));
  ...
  return (
    <main className="w-full mx-auto flex flex-col gap-4 xs:p-0 md:p-8 bg-background-primary overflow-hidden">
      ...
      {/* Section Title */}
      <TitleSectionAdmin label="Tipe Pelatihan" href="/tipe-pelatihan/tambah" />
      ...
      <section className="flex xs:flex-col xs:justify-start xs:items-start md:flex-row md:justify-between md:items-center gap-2 border-b border-border-primary pb-4">
        ...
        <Input
          ...
          label="Cari informasi tipe pelatihan"
      ...
      <div className="flex flex-col gap-2">
        {searchKeyword && (
          <div className="flex flex-col">
            ...
            <span className="text-xs text-text-secondary">
              Kami menemukan <span className="font-bold text-primary-primary">{data.length}</span> informasi tipe pelatihan
            </span>
          </div>
        )}
      </div>
      ...
      {viewMode === "card" ? (
        <section className="grid xs:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {isLoading ? (
            ...
            currentItems.map((item) => <CardTipePelatihan key={item.training_type_id} {...item} />)
          )}
        </section>
      ) : (
        <Table
                      ...
                      <TableCell>
                        <div className="flex justify-center items-center gap-2">
                          <Tooltip content="Lihat detail" placement="top" className="bg-yellow-500 text-white text-xs px-2 py-1 rounded">
                            <Button as={Link} isIconOnly variant="light" size="sm" href={`/tipe-pelatihan/${data.training_type_id}`}>
                          ...
                          <Tooltip content="Edit data" placement="top" className="bg-primary-primary text-white text-xs px-2 py-1 rounded">
                            <Button as={Link} isIconOnly variant="light" size="sm" href={`/tipe-pelatihan/edit/${data.training_type_id}`}>
                            ...
                            <Button
                              ...
                              onPress={async () => {
                                ...
                                if (confirm.isConfirmed) {
                                  const result = await deleteTrainingTypeById(data.training_type_id);
                                  if (result.success) {
