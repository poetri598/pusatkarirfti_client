...
import CardPengalaman from "@/components/Card/CardPengalaman";
...
// Services
import { searchExperiences, deleteExperienceById } from "@/services/experience";
// Utils
...
export default function page() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const { success, data, error } = await searchExperiences(queryParams);
        if (success) {
          setData(data || []);
        } else {
  ...
  const sortOptions = [
    ...
    { key: "experience_name:asc", label: "A-Z" },
    { key: "experience_name:desc", label: "Z-A" },
    { key: "experience_created_at:desc", label: "Terbaru" },
    { key: "experience_created_at:asc", label: "Terlama" },
    { key: "experience_updated_at:desc", label: "Diubah Terakhir" },
    { key: "experience_updated_at:asc", label: "Diubah Terawal" },
  ];
  ...
  const columns = [
    { key: "no", label: "#" },
    { key: "experience_name", label: "Nama Pengalaman" },
    { key: "experience_created_at", label: "Tanggal Dibuat" },
  ...
  const tableItems = currentItems.map((item, index) => ({
    ...
    experience_created_at: getRelativeTimeRaw(item.experience_created_at),
    actions: item,
  }));
  ...
  return (
    <main className="w-full mx-auto flex flex-col gap-4 xs:p-0 md:p-8 bg-background-primary overflow-hidden">
      ...
      {/* Section Title */}
      <TitleSectionAdmin label="Pengalaman" href="/pengalaman/tambah" />
      ...
      <section className="flex xs:flex-col xs:justify-start xs:items-start md:flex-row md:justify-between md:items-center gap-2 border-b border-border-primary pb-4">
        ...
        <Input
          ...
          label="Cari informasi pengalaman"
      ...
      <div className="flex flex-col gap-2">
        {searchKeyword && (
          <div className="flex flex-col">
            ...
            <span className="text-xs text-text-secondary">
              Kami menemukan <span className="font-bold text-primary-primary">{data.length}</span> informasi pengalaman
            </span>
          </div>
        )}
      </div>
      ...
      {viewMode === "card" ? (
        <section className="grid xs:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {isLoading ? (
            ...
            currentItems.map((item) => <CardPengalaman key={item.experience_id} {...item} />)
          )}
        </section>
      ) : (
        <Table
                      ...
                      <TableCell>
                        <div className="flex justify-center items-center gap-2">
                          <Tooltip content="Lihat detail" placement="top" className="bg-yellow-500 text-white text-xs px-2 py-1 rounded">
                            <Button as={Link} isIconOnly variant="light" size="sm" href={`/pengalaman/${data.experience_id}`}>
                          ...
                          <Tooltip content="Edit data" placement="top" className="bg-primary-primary text-white text-xs px-2 py-1 rounded">
                            <Button as={Link} isIconOnly variant="light" size="sm" href={`/pengalaman/edit/${data.experience_id}`}>
                            ...
                            <Button
                              ...
                              onPress={async () => {
                                ...
                                if (confirm.isConfirmed) {
                                  const result = await deleteExperienceById(data.experience_id);
                                  if (result.success) {
