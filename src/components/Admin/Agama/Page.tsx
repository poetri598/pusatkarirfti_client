...
import CardAgama from "@/components/Card/CardAgama";
...
// Services
import { searchReligions, deleteReligionById } from "@/services/religion";
// Utils
import { getRelativeTimeRaw } from "@/utils/time";

export default function page() {
  const [data, setData] = useState<ReligionItem[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const { success, data, error } = await searchReligions(queryParams);
        if (success) {
          setData(data || []);
        } else {
  ...
  const sortOptions = [
    ...
    { key: "religion_name:asc", label: "A-Z" },
    { key: "religion_name:desc", label: "Z-A" },
    { key: "religion_created_at:desc", label: "Terbaru" },
    { key: "religion_created_at:asc", label: "Terlama" },
    { key: "religion_updated_at:desc", label: "Diubah Terakhir" },
    { key: "religion_updated_at:asc", label: "Diubah Terawal" },
  ];
  ...
  const columns = [
    { key: "no", label: "#" },
    { key: "religion_name", label: "Nama Agama" },
    { key: "religion_created_at", label: "Tanggal Dibuat" },
  ...
  const tableItems = currentItems.map((item, index) => ({
    ...
    religion_created_at: getRelativeTimeRaw(item.religion_created_at),
    actions: item,
  }));
  ...
  return (
    <main className="w-full mx-auto flex flex-col gap-4 xs:p-0 md:p-8 bg-background-primary overflow-hidden">
      ...
      >
        ...
        <BreadcrumbItem href="/agama">Agama</BreadcrumbItem>
      </Breadcrumbs>

      {/* Section Title */}
      <TitleSectionAdmin label="Agama" href="/agama/tambah" />
      ...
      <div className="flex flex-col gap-2">
        {searchKeyword && (
          <div className="flex flex-col">
            ...
            <span className="text-xs text-text-secondary">
              Kami menemukan <span className="font-bold text-primary-primary">{data.length}</span> informasi agama
            </span>
          </div>
        )}
      </div>
      ...
      {viewMode === "card" ? (
        <section className="grid xs:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {isLoading ? (
            ...
            currentItems.map((item) => <CardAgama key={item.religion_id} {...item} />)
          )}
        </section>
      ) : (
        <Table
                      ...
                      <TableCell>
                        <div className="flex justify-center items-center gap-2">
                          <Tooltip content="Lihat detail" placement="top" className="bg-yellow-500 text-white text-xs px-2 py-1 rounded">
                            <Button as={Link} isIconOnly variant="light" size="sm" href={`/agama/${data.religion_id}`}>
                          ...
                          <Tooltip content="Edit data" placement="top" className="bg-primary-primary text-white text-xs px-2 py-1 rounded">
                            <Button as={Link} isIconOnly variant="light" size="sm" href={`/agama/edit/${data.religion_id}`}>
                            ...
                            <Button
                              ...
                              onPress={async () => {
                                ...
                                if (confirm.isConfirmed) {
                                  const result = await deleteReligionById(data.religion_id);
                                  if (result.success) {
