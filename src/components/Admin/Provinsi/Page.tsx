...
import CardProvinsi from "@/components/Card/CardProvinsi";
...
// Services
import { searchProvinces, deleteProvinceById } from "@/services/province";
// Utils
import { getRelativeTimeRaw } from "@/utils/time";

export default function page() {
  const [data, setData] = useState<ProvinceItem[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const { success, data, error } = await searchProvinces(queryParams);
        if (success) {
          setData(data || []);
        } else {
  ...
  const sortOptions = [
    ...
    { key: "province_name:asc", label: "A-Z" },
    { key: "province_name:desc", label: "Z-A" },
    { key: "province_created_at:desc", label: "Terbaru" },
    { key: "province_created_at:asc", label: "Terlama" },
    { key: "province_updated_at:desc", label: "Diubah Terakhir" },
    { key: "province_updated_at:asc", label: "Diubah Terawal" },
  ];
  ...
  const columns = [
    ...
    { key: "province_created_at", label: "Tanggal Dibuat" },
  ...
  const tableItems = currentItems.map((item, index) => ({
    ...
    province_created_at: getRelativeTimeRaw(item.province_created_at),
    actions: item,
  }));
  ...
  return (
    <main className="w-full mx-auto flex flex-col gap-4 xs:p-0 md:p-8 bg-background-primary overflow-hidden">
      ...
      {/* Section Title */}
      <TitleSectionAdmin label="Provinsi" href="/provinsi/tambah" />
      ...
      <div className="flex flex-col gap-2">
        {searchKeyword && (
          <div className="flex flex-col">
            ...
            <span className="text-xs text-text-secondary">
              Kami menemukan <span className="font-bold text-primary-primary">{data.length}</span> informasi provinsi
            </span>
          </div>
        )}
      </div>
      ...
      {viewMode === "card" ? (
        <section className="grid xs:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {isLoading ? (
            ...
            currentItems.map((item) => <CardProvinsi key={item.province_id} {...item} />)
          )}
        </section>
      ) : (
        <Table
                      ...
                      <TableCell>
                        <div className="flex justify-center items-center gap-2">
                          <Tooltip content="Lihat detail" placement="top" className="bg-yellow-500 text-white text-xs px-2 py-1 rounded">
                            <Button as={Link} isIconOnly variant="light" size="sm" href={`/provinsi/${data.province_id}`}>
                          ...
                          <Tooltip content="Edit data" placement="top" className="bg-primary-primary text-white text-xs px-2 py-1 rounded">
                            <Button as={Link} isIconOnly variant="light" size="sm" href={`/provinsi/edit/${data.province_id}`}>
                            ...
                            <Button
                              ...
                              onPress={async () => {
                                ...
                                if (confirm.isConfirmed) {
                                  const result = await deleteProvinceById(data.province_id);
                                  if (result.success) {
