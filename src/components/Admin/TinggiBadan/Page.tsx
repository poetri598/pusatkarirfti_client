...
import CardTinggiBadan from "@/components/Card/CardTinggiBadan";
...
// Services
import { searchHeights, deleteHeightById } from "@/services/height";
// Utils
import { getRelativeTimeRaw } from "@/utils/time";

export default function page() {
  const [data, setData] = useState<HeightItem[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const { success, data, error } = await searchHeights(queryParams);
        if (success) {
          setData(data || []);
        } else {
  ...
  const sortOptions = [
    ...
    { key: "height_no:asc", label: "A-Z" },
    { key: "height_no:desc", label: "Z-A" },
    { key: "height_created_at:desc", label: "Terbaru" },
    { key: "height_created_at:asc", label: "Terlama" },
    { key: "height_updated_at:desc", label: "Diubah Terakhir" },
    { key: "height_updated_at:asc", label: "Diubah Terawal" },
  ];
  ...
  const columns = [
    { key: "no", label: "#" },
    { key: "height_no", label: "Nomor Tinggi Badan" },
    { key: "height_created_at", label: "Tanggal Dibuat" },
  ...
  const tableItems = currentItems.map((item, index) => ({
    key: item.height_id,
    no: index + 1,
    height_no: item.height_no,
    height_created_at: getRelativeTimeRaw(item.height_created_at),
    actions: item,
  }));
  ...
  return (
    <main className="w-full mx-auto flex flex-col gap-4 xs:p-0 md:p-8 bg-background-primary overflow-hidden">
      ...
      >
        ...
        <BreadcrumbItem href="/tinggi-badan">Tinggi Badan</BreadcrumbItem>
      </Breadcrumbs>

      {/* Section Title */}
      <TitleSectionAdmin label="Tinggi Badan" href="/tinggi-badan/tambah" />
      ...
      <section className="flex xs:flex-col xs:justify-start xs:items-start md:flex-row md:justify-between md:items-center gap-2 border-b border-border-primary pb-4">
        ...
        <Input
          ...
          label="Cari informasi tinggi badan"
      ...
      <div className="flex flex-col gap-2">
        {searchKeyword && (
          <div className="flex flex-col">
            ...
            <span className="text-xs text-text-secondary">
              Kami menemukan <span className="font-bold text-primary-primary">{data.length}</span> informasi tinggi badan
            </span>
          </div>
        )}
      </div>
      ...
      {viewMode === "card" ? (
        <section className="grid xs:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {isLoading ? (
            ...
            currentItems.map((item) => <CardTinggiBadan key={item.height_id} {...item} />)
          )}
        </section>
      ) : (
        <Table
                      ...
                      <TableCell>
                        <div className="flex justify-center items-center gap-2">
                          <Tooltip content="Lihat detail" placement="top" className="bg-yellow-500 text-white text-xs px-2 py-1 rounded">
                            <Button as={Link} isIconOnly variant="light" size="sm" href={`/perusahaan/${data.height_id}`}>
                          ...
                          <Tooltip content="Edit data" placement="top" className="bg-primary-primary text-white text-xs px-2 py-1 rounded">
                            <Button as={Link} isIconOnly variant="light" size="sm" href={`/perusahaan/edit/${data.height_id}`}>
                            ...
                            <Button
                              ...
                              onPress={async () => {
                                ...
                                if (confirm.isConfirmed) {
                                  const result = await deleteHeightById(data.height_id);
                                  if (result.success) {
