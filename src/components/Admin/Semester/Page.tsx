...
import CardSemester from "@/components/Card/CardSemester";
...
// Services
import { searchSemesters, deleteSemesterById } from "@/services/semester";
// Utils
...
export default function page() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const { success, data, error } = await searchSemesters(queryParams);
        if (success) {
          setData(data || []);
        } else {
  ...
  const sortOptions = [
    ...
    { key: "semester_no:asc", label: "A-Z" },
    { key: "semester_no:desc", label: "Z-A" },
    { key: "semester_created_at:desc", label: "Terbaru" },
    { key: "semester_created_at:asc", label: "Terlama" },
    { key: "semester_updated_at:desc", label: "Diubah Terakhir" },
    { key: "semester_updated_at:asc", label: "Diubah Terawal" },
  ];
  ...
  const columns = [
    { key: "no", label: "#" },
    { key: "semester_no", label: "Nomor Semester" },
    { key: "semester_created_at", label: "Tanggal Dibuat" },
  ...
  const tableItems = currentItems.map((item, index) => ({
    ...
    semester_created_at: getRelativeTimeRaw(item.semester_created_at),
    actions: item,
  }));
  ...
  return (
    <main className="w-full mx-auto flex flex-col gap-4 xs:p-0 md:p-8 bg-background-primary overflow-hidden">
      ...
      {/* Section Title */}
      <TitleSectionAdmin label="Semester" href="/semester/tambah" />
      ...
      <div className="flex flex-col gap-2">
        {searchKeyword && (
          <div className="flex flex-col">
            ...
            <span className="text-xs text-text-secondary">
              Kami menemukan <span className="font-bold text-primary-primary">{data.length}</span> informasi semester
            </span>
          </div>
        )}
      </div>
      ...
      {viewMode === "card" ? (
        <section className="grid xs:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {isLoading ? (
            ...
            currentItems.map((item) => <CardSemester key={item.semester_id} {...item} />)
          )}
        </section>
      ) : (
        <Table
                      ...
                      <TableCell>
                        <div className="flex justify-center items-center gap-2">
                          <Tooltip content="Lihat detail" placement="top" className="bg-yellow-500 text-white text-xs px-2 py-1 rounded">
                            <Button as={Link} isIconOnly variant="light" size="sm" href={`/perusahaan/${data.semester_id}`}>
                          ...
                          <Tooltip content="Edit data" placement="top" className="bg-primary-primary text-white text-xs px-2 py-1 rounded">
                            <Button as={Link} isIconOnly variant="light" size="sm" href={`/perusahaan/edit/${data.semester_id}`}>
                            ...
                            <Button
                              ...
                              onPress={async () => {
                                ...
                                if (confirm.isConfirmed) {
                                  const result = await deleteSemesterById(data.semester_id);
                                  if (result.success) {
