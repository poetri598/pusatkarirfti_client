...
import { getSemesterById } from "@/services/semester";

export default function page({ semester_id }: { semester_id: number }) {
  const [data, setData] = useState<SemesterItem | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const { success, data, error } = await getSemesterById(semester_id);
        if (success && data) {
          setData(data);
        } else {
          setData(null);
    ...
    <main className="xs:p-0 md:p-8  flex flex-col xs:gap-2 md:gap-8 overflow-hidden">
      <section className="flex flex-col xs:gap-2">
        ...
        >
          ...
          <BreadcrumbItem href={`/semester/${semester_id}`}>Detail Semester</BreadcrumbItem>
        </Breadcrumbs>

        <section className="flex flex-col items-center justify-center xs:gap-2 min-h-screen">
          <span className="xs:text-xs md:text-9xl font-bold text-text-primary">{data.semester_no}</span>
          {/* Tanggal */}
          <div className="flex items-center">
            {" "}
            <Clock size={20} color="currentColor" variant="Bulk" className="text-primary-primary" />
            <Tooltip
              content={getFullTimeRaw(data.semester_created_at)}
              placement="top"
              classNames={{
                content: "text-xs text-background-primary bg-primary-primary",
              }}
            >
              <span className="text-xs text-text-secondary cursor-help">{getRelativeTimeRaw(data.semester_created_at)}</span>
            </Tooltip>
          </div>
        </section>
      </section>
