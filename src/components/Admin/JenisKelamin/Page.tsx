...
import CardJenisKelamin from "@/components/Card/CardJenisKelamin";
...
// Services
import { searchGenders, deleteGenderById } from "@/services/gender";
// Utils
...
export default function page() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const { success, data, error } = await searchGenders(queryParams);
        if (success) {
          setData(data || []);
        } else {
  ...
  const sortOptions = [
    ...
    { key: "gender_name:asc", label: "A-Z" },
    { key: "gender_name:desc", label: "Z-A" },
    { key: "gender_created_at:desc", label: "Terbaru" },
    { key: "gender_created_at:asc", label: "Terlama" },
    { key: "gender_updated_at:desc", label: "Diubah Terakhir" },
    { key: "gender_updated_at:asc", label: "Diubah Terawal" },
  ];
  ...
  const columns = [
    { key: "no", label: "#" },
    { key: "gender_name", label: "Nama Jenis Kelamin" },
    { key: "gender_created_at", label: "Tanggal Dibuat" },
  ...
  const tableItems = currentItems.map((item, index) => ({
    ...
    gender_created_at: getRelativeTimeRaw(item.gender_created_at),
    actions: item,
  }));
  ...
  return (
    <main className="w-full mx-auto flex flex-col gap-4 xs:p-0 md:p-8 bg-background-primary overflow-hidden">
      ...
      {/* Section Title */}
      <TitleSectionAdmin label="Jenis Kelamin" href="/jenis-kelamin/tambah" />
      ...
      <section className="flex xs:flex-col xs:justify-start xs:items-start md:flex-row md:justify-between md:items-center gap-2 border-b border-border-primary pb-4">
        ...
        <Input
          ...
          label="Cari informasi jenis kelamin"
      ...
      <div className="flex flex-col gap-2">
        {searchKeyword && (
          <div className="flex flex-col">
            ...
            <span className="text-xs text-text-secondary">
              Kami menemukan <span className="font-bold text-primary-primary">{data.length}</span> informasi jenis kelamin
            </span>
          </div>
        )}
      </div>
      ...
      {viewMode === "card" ? (
        <section className="grid xs:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {isLoading ? (
            ...
            currentItems.map((item) => <CardJenisKelamin key={item.gender_id} {...item} />)
          )}
        </section>
      ) : (
        <Table
                      ...
                      <TableCell>
                        <div className="flex justify-center items-center gap-2">
                          <Tooltip content="Lihat detail" placement="top" className="bg-yellow-500 text-white text-xs px-2 py-1 rounded">
                            <Button as={Link} isIconOnly variant="light" size="sm" href={`/jenis-kelamin/${data.gender_id}`}>
                          ...
                          <Tooltip content="Edit data" placement="top" className="bg-primary-primary text-white text-xs px-2 py-1 rounded">
                            <Button as={Link} isIconOnly variant="light" size="sm" href={`/jenis-kelamin/edit/${data.gender_id}`}>
                            ...
                            <Button
                              ...
                              onPress={async () => {
                                ...
                                if (confirm.isConfirmed) {
                                  const result = await deleteGenderById(data.gender_id);
                                  if (result.success) {
