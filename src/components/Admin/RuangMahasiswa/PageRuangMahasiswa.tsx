...
import CardRuangMahasiswa from "@/components/Card/CardRuangMahasiswa";
...
import { searchStudentRooms, deleteStudentRoomById } from "@/services/studentRoom";
// Utils
...
export default function PageMagang() {
  ...
  const [student_rooms, setStudentRooms] = useState<StudentRoomItem[]>([]);
  const [isLoadingAllStudentRooms, setIsLoadingAllStudentRooms] = useState(true);
  const [apiErrorAllStudentRooms, setApiErrorAllStudentRooms] = useState<string | null>(null);

  const [currentPage, setCurrentPage] = useState(1);
  const [amount, setAmount] = useState(8);

  const maxValue = student_rooms.length;
  const minValue = 1;
  const currentItems = student_rooms.slice((currentPage - 1) * amount, currentPage * amount);
  ...
  useEffect(() => {
    const fetchStudentRooms = async () => {
      setIsLoadingAllStudentRooms(true);
      setApiErrorAllStudentRooms(null);
      ...
      try {
        ...
        const { success, data, error } = await searchStudentRooms(queryParams);
        if (success) {
          setStudentRooms(data || []);
        } else {
          setApiErrorAllStudentRooms(error || "Terjadi kesalahan");
        }
      } catch (err: any) {
        setApiErrorAllStudentRooms(err.message || "Terjadi kesalahan");
      } finally {
        setIsLoadingAllStudentRooms(false);
      }
    };
  ...
  const sortOptions = [
    ...
    { key: "student_room_created_at:desc", label: "Terbaru" },
    { key: "student_room_created_at:asc", label: "Terlama" },
  ];
  ...
  const columns = [
    { key: "no", label: "#" },
    { key: "user_fullname", label: "Nama Pengguna" },
    { key: "internship_created_at", label: "Tanggal Dibuat" },
    { key: "actions", label: "Aksi" },
  ];

  const tableItems = currentItems.map((item, index) => ({
    key: item.student_room_id,
    no: index + 1,
    user_fullname: item.user_fullname.length > 30 ? item.user_fullname.slice(0, 30) + "..." : item.user_fullname,
    internship_created_at: getRelativeTimeRaw(item.student_room_created_at),
    actions: item,
  }));
  ...
  return (
    <main className="w-full mx-auto flex flex-col gap-4 xs:p-0 md:p-8 bg-background-primary overflow-hidden">
      ...
      {/* Section Title */}
      <TitleSectionAdmin label="Ruang Mahasiswa" href="/ruang-mahasiswa/tambah" />
      ...
      <section className="flex xs:flex-col xs:justify-start xs:items-start md:flex-row md:justify-between md:items-center gap-2 border-b border-border-primary pb-4">
        ...
        <Input
          ...
          label="Cari informasi ruang mahasiswa"
      ...
      <div className="flex flex-col gap-2">
        {(searchKeyword || Object.keys(filters).length > 0) && (
          <div className="flex flex-col">
            ...
            <span className="text-xs text-text-secondary">
              Kami menemukan <span className="font-bold text-primary-primary">{student_rooms.length}</span> ruang mahasiswa
            </span>
          </div>
        )}
      </div>
      ...
      {viewMode === "card" ? (
        <section className="grid xs:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          ) : apiErrorAllStudentRooms ? (
            <p className="text-start text-xs text-danger-primary">{apiErrorAllStudentRooms}</p>
          ) : currentItems.length === 0 ? (
            <div className="w-full col-span-full text-center py-8">
              <p className="text-sm text-gray-500">Data belum tersedia.</p>
            </div>
          ) : (
            currentItems.map((item) => <CardRuangMahasiswa key={item.student_room_id} {...item} />)
          )}
        </section>
      ) : (
        <Table
              ...
              <TableRow key={item.key}>
                {(columnKey) => {
                  if (columnKey === "actions") {
                    const student_room = item.actions;
                    return (
                      <TableCell>
                        <div className="flex justify-center items-center gap-2">
                          <Tooltip content="Lihat detail" placement="top" className="bg-yellow-500 text-white text-xs px-2 py-1 rounded">
                            <Button as={Link} isIconOnly variant="light" size="sm" href={`/ruang-mahasiswa/${student_room.student_room_id}`}>
                          ...
                          <Tooltip content="Edit data" placement="top" className="bg-primary-primary text-white text-xs px-2 py-1 rounded">
                            <Button as={Link} isIconOnly variant="light" size="sm" href={`/ruang-mahasiswa/edit/${student_room.student_room_id}`}>
                            ...
                            <Button
                              ...
                              onPress={async () => {
                                ...
                                if (confirm.isConfirmed) {
                                  const result = await deleteStudentRoomById(student_room.student_room_id);
                                  if (result.success) {
