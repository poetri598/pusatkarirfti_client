...
// Components
import { Breadcrumbs, BreadcrumbItem, Input, NumberInput, Pagination, Button, Spinner, Select, SelectItem, Selection, Tooltip, Link, Table, TableHeader, TableColumn, TableBody, TableRow, TableCell, getKeyValue, Chip } from "@heroui/react";
...
// Services
import { searchPlatforms, deletePlatformById } from "@/services/platform";
// Utils
...
export default function page() {
  ...
  const [PlatfsearchPlatforms, setPlatfsearchPlatforms] = useState<PlatformItem[]>([]);
  const [isLoadingAllPlatfsearchPlatforms, setIsLoadingAllPlatfsearchPlatforms] = useState(true);
  const [apiErrorAllPlatfsearchPlatforms, setApiErrorAllPlatfsearchPlatforms] = useState<string | null>(null);

  const [currentPage, setCurrentPage] = useState(1);
  const [amount, setAmount] = useState(6);

  const maxValue = PlatfsearchPlatforms.length;
  const minValue = 1;
  const currentItems = PlatfsearchPlatforms.slice((currentPage - 1) * amount, currentPage * amount);
  const totalPage = Math.ceil(maxValue / amount);

  useEffect(() => {
    const fetchPlatfsearchPlatforms = async () => {
      setIsLoadingAllPlatfsearchPlatforms(true);
      setApiErrorAllPlatfsearchPlatforms(null);
      ...
      try {
        ...
        const { success, data, error } = await searchPlatforms(queryParams);
        if (success) {
          setPlatfsearchPlatforms(data || []);
        } else {
          setApiErrorAllPlatfsearchPlatforms(error || "Terjadi kesalahan");
        }
      } catch (err: any) {
        setApiErrorAllPlatfsearchPlatforms(err.message || "Terjadi kesalahan");
      } finally {
        setIsLoadingAllPlatfsearchPlatforms(false);
      }
    };

    fetchPlatfsearchPlatforms();
  }, [searchKeyword, sort, filters]);

  const sortOptions = [
    { key: "", label: "— Tidak diurutkan —" },
    { key: "platform_name:asc", label: "A-Z" },
    { key: "platform_name:desc", label: "Z-A" },
    { key: "platform_created_at:desc", label: "Terbaru" },
    { key: "platform_created_at:asc", label: "Terlama" },
    { key: "platform_updated_at:desc", label: "Diubah Terakhir" },
    { key: "platform_updated_at:asc", label: "Diubah Terawal" },
  ];
  ...
  const columns = [
    ...
    { key: "platform_created_at", label: "Tanggal Dibuat" },
  ...
  const tableItems = currentItems.map((item, index) => ({
    ...
    no: index + 1,
    platform_name: item.platform_name.length > 30 ? item.platform_name.slice(0, 30) + "..." : item.platform_name,
    platform_created_at: getRelativeTimeRaw(item.platform_created_at),
    actions: item,
  }));
  ...
  return (
    <main className="w-full mx-auto flex flex-col gap-4 xs:p-0 md:p-8 bg-background-primary overflow-hidden">
      ...
      {/* Section Title */}
      <TitleSectionAdmin label="Platform" href="/platform/tambah" />
      ...
      <div className="flex flex-col gap-2">
        {(searchKeyword || Object.keys(filters).length > 0) && (
          <div className="flex flex-col">
            ...
            <span className="text-xs text-text-secondary">
              Kami menemukan <span className="font-bold text-primary-primary">{PlatfsearchPlatforms.length}</span> Platform
            </span>
          </div>
        )}
      </div>
      ...
      {viewMode === "card" ? (
        <section className="grid xs:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {isLoadingAllPlatfsearchPlatforms ? (
          ...
          ) : apiErrorAllPlatfsearchPlatforms ? (
            <p className="text-start text-xs text-danger-primary">{apiErrorAllPlatfsearchPlatforms}</p>
          ) : currentItems.length === 0 ? (
            <div className="w-full col-span-full text-center py-8">
              <p className="text-sm text-gray-500">Data belum tersedia.</p>
            </div>
          ) : (
            currentItems.map((item) => <CardPlatform key={item.platform_id} {...item} />)
          )}
        </section>
      ) : (
        <Table
                      ...
                      <TableCell>
                        <div className="flex justify-center items-center gap-2">
                          <Tooltip content="Lihat detail" placement="top" className="bg-yellow-500 text-white text-xs px-2 py-1 rounded">
                            <Button as={Link} isIconOnly variant="light" size="sm" href={`/platform/${data.platform_id}`}>
                          ...
                          <Tooltip content="Edit data" placement="top" className="bg-primary-primary text-white text-xs px-2 py-1 rounded">
                            <Button as={Link} isIconOnly variant="light" size="sm" href={`/platform/edit/${data.platform_id}`}>
                            ...
                            <Button
                              ...
                              onPress={async () => {
                                ...
                                if (confirm.isConfirmed) {
                                  const result = await deletePlatformById(data.platform_id);
                                  if (result.success) {
