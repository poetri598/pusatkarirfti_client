...
// Services
import { getSkillLevelById, updateSkillLevelById } from "@/services/skillLevel";

export default function Edit({ skill_level_id }: { skill_level_id: number }) {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      ...
      setSkillLevelName(data?.skill_level_name || "");
      if (!success || !data) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { success, error } = await updateSkillLevelById(skill_level_id, formData);
    if (success) {
      ...
      <>
        <main className="xs:p-0 md:p-8  flex flex-col xs:gap-2 md:gap-8 overflow-hidden">
          ...
          >
            ...
            <BreadcrumbItem href={`/skill-level/edit/${skill_level_id}`}>Ubah Data Skill Level</BreadcrumbItem>
          </Breadcrumbs>

          {/* Section Title */}
          <TitleSectionAdmin label="Ubah Data Skill Level" />
