...
import { updateUserById, getUserByUsername } from "@/services/user";
...
export default function Edit({ user_name }: { user_name: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { success, error } = await updateUserById(user_id, formData);
    if (success) {
      ...
      <>
        <main className="xs:p-0 md:p-8  flex flex-col xs:gap-2 md:gap-8 overflow-hidden">
          ...
          >
            ...
            <BreadcrumbItem href="/pengguna/ubah">Ubah Data Pengguna</BreadcrumbItem>
          </Breadcrumbs>

          {/* Section Title */}
          <TitleSectionAdmin label="Ubah data pengguna" />
