...
// Services
import { getProvinceById, updateProvinceById } from "@/services/province";

export default function Edit({ province_id }: { province_id: number }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { success, error } = await updateProvinceById(province_id, formData);
    if (success) {
      ...
      <>
        <main className="xs:p-0 md:p-8  flex flex-col xs:gap-2 md:gap-8 overflow-hidden">
          ...
          >
            ...
            <BreadcrumbItem href={`/provinsi/edit/${province_id}`}>Ubah Data Provinsi</BreadcrumbItem>
          </Breadcrumbs>

          {/* Section Title */}
          <TitleSectionAdmin label="Ubah Data Provinsi" />
