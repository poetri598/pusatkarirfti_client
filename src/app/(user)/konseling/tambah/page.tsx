...
export default function page() {
  ...
  const [counseling_type_id, setJobTypeId] = useState<Selection>(new Set([]));
  ...
  useEffect(() => {
    const fetchAll = async () => {
      const fetchers = [
        ...
        createFetcher<UserItem[]>("/users", setUsers, setApiErrorUsers, setIsLoadingUsers),
      ];
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // SINGLE
    if (user_id instanceof Set && user_id.size > 0) {
      formData.append("user_id", String(Array.from(user_id)[0]));
    }

    if (counseling_type_id instanceof Set && counseling_type_id.size > 0) {
      formData.append("counseling_type_id", String(Array.from(counseling_type_id)[0]));
    }
