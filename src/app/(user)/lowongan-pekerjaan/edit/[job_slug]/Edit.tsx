...
export default function Edit({ job_slug }: { job_slug: string }) {
  ...
  const [job_id, setJobId] = useState<number>(0);
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      const { success, data, error } = await getJobBySlug(job_slug);
      if (!success || !data) {
        await showErrorDialog(error || "Gagal mengambil data pekerjaan.");
        return;
      }
      setJobId(data.job_id);
      if (data.job_img) setJobImg(data.job_img);
      setJobName(data.job_name);
      setJobDesc(data.job_desc);
      setJobSalaryMin(Number(data.job_salary_min));
      setJobSalaryMax(Number(data.job_salary_max));
      setJobLocation(data.job_location);
      setJobLink(data.job_link);
      if (data.job_open_date) setJobOpenDate(parseAbsoluteToLocal(data.job_open_date));
      if (data.job_close_date) setJobCloseDate(parseAbsoluteToLocal(data.job_close_date));
      setAgeMinId(new Set([String(data.age_min_id)]));
      setAgeMaxId(new Set([String(data.age_max_id)]));
      setWeightMinId(new Set([String(data.weight_min_id)]));
      setWeightMaxId(new Set([String(data.weight_max_id)]));
      setHeightMinId(new Set([String(data.height_min_id)]));
      setHeightMaxId(new Set([String(data.height_max_id)]));
      setJobTypeId(new Set([String(data.job_type_id)]));
      ...
      setExperienceIds(new Set(String(data.experience_ids).split(",")));
      setGenderIds(new Set(String(data.gender_ids).split(",")));
      setMaritalStatusIds(new Set(String(data.marital_status_ids).split(",")));
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const { success, error } = await updateJobById(job_id, formData);
    if (success) {
      ...
      <>
        <main className="xs:p-0 md:p-8  flex flex-col xs:gap-2 md:gap-8 overflow-hidden">
          ...
          >
            ...
            <BreadcrumbItem href="/lowongan-pekerjaan/edit">Ubah Data Lowongan Pekerjaan</BreadcrumbItem>
          </Breadcrumbs>
