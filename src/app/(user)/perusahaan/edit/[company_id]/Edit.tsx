...
export default function page({ company_id }: { company_id: number }) {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      ...
      if (success && data) {
        if (data.company_img) setTrainingImg(data.company_img);
        setCompanyName(data.company_name);
        setCompanyDesc(data.company_desc);
        setCompanyLink(data.company_link);
        setCompanyIsPartner(data.company_is_partner);
        setStatusId(new Set([String(data.status_id)]));
        console.log(data.industry_ids);
        setIndustryIds(new Set(String(data.industry_ids).split(",")));
      }
      setLoading(false);
    };
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const { success, error } = await updateCompanyById(company_id, formData);
    if (success) {
      ...
      <>
        <main className="xs:p-0 md:p-8  flex flex-col xs:gap-2 md:gap-8 overflow-hidden">
          ...
          >
            ...
            <BreadcrumbItem href={`/perusahaan/edit/${company_id}`}>Ubah Data Perusahaan</BreadcrumbItem>
          </Breadcrumbs>

          {/* Section Title */}
          <TitleSectionAdmin label="Ubah Data Perusahaan" />
