...
export default function page() {
  ...
  const [platform_img, setTrainingImg] = useState<string>("/tambah-bg.png");
  const [platform_img_file, setTrainingImgFile] = useState<File | null>(null);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const { success, error } = await createPlatform(formData);
    if (success) {
      ...
      <>
        <main className="xs:p-0 md:p-8  flex flex-col xs:gap-2 md:gap-8 overflow-hidden">
          ...
          >
            ...
            <BreadcrumbItem href="/platform/tambah">Tambah Platform</BreadcrumbItem>
          </Breadcrumbs>

          {/* Section Title */}
          <TitleSectionAdmin label="Tambah Platform" />
