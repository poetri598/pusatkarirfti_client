...
// Services
import { getGenderById, updateGenderById } from "@/services/gender";

export default function Edit({ gender_id }: { gender_id: number }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { success, error } = await updateGenderById(gender_id, formData);
    if (success) {
      ...
      <>
        <main className="xs:p-0 md:p-8  flex flex-col xs:gap-2 md:gap-8 overflow-hidden">
          ...
          >
            ...
            <BreadcrumbItem href={`/jenis-kelamin/edit/${gender_id}`}>Ubah Data Jenis Kelamin</BreadcrumbItem>
          </Breadcrumbs>

          {/* Section Title */}
          <TitleSectionAdmin label="Ubah Data Jenis Kelamin" />
