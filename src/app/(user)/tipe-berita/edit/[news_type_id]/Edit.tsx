...
// Services
import { getNewsTypeById, updateNewsTypeById } from "@/services/newsType";

export default function Edit({ news_type_id }: { news_type_id: number }) {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      ...
      setNewsTypeName(data?.news_type_name || "");
      if (!success || !data) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { success, error } = await updateNewsTypeById(news_type_id, formData);
    if (success) {
      ...
      <>
        <main className="xs:p-0 md:p-8  flex flex-col xs:gap-2 md:gap-8 overflow-hidden">
          ...
          >
            ...
            <BreadcrumbItem href={`/tipe-berita/edit/${news_type_id}`}>Ubah Data Tipe Berita</BreadcrumbItem>
          </Breadcrumbs>

          {/* Section Title */}
          <TitleSectionAdmin label="Ubah Data Tipe Berita" />
