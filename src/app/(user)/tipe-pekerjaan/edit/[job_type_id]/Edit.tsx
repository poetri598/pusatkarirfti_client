...
// Services
import { getJobTypeById, updateJobTypeById } from "@/services/jobType";

export default function Edit({ job_type_id }: { job_type_id: number }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { success, error } = await updateJobTypeById(job_type_id, formData);
    if (success) {
      ...
      <>
        <main className="xs:p-0 md:p-8  flex flex-col xs:gap-2 md:gap-8 overflow-hidden">
          ...
          >
            ...
            <BreadcrumbItem href={`/tipe-pekerjaan/edit/${job_type_id}`}>Ubah Data Tipe Pekerjaan</BreadcrumbItem>
          </Breadcrumbs>

          {/* Section Title */}
          <TitleSectionAdmin label="Ubah Data Tipe Pekerjaan" />
