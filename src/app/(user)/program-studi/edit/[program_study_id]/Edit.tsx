...
// Services
import { getProgramStudyById, updateProgramStudyById } from "@/services/programStudy";

export default function Edit({ program_study_id }: { program_study_id: number }) {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      ...
      setProgramStudyName(data?.program_study_name || "");
      if (!success || !data) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { success, error } = await updateProgramStudyById(program_study_id, formData);
    if (success) {
      ...
      <>
        <main className="xs:p-0 md:p-8  flex flex-col xs:gap-2 md:gap-8 overflow-hidden">
          ...
          >
            ...
            <BreadcrumbItem href={`/program-studi/edit/${program_study_id}`}>Ubah Data Program Studi</BreadcrumbItem>
          </Breadcrumbs>

          {/* Section Title */}
          <TitleSectionAdmin label="Ubah Data Program Studi" />
